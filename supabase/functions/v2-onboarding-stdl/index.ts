  ...
  for (const action of ATOMIC_ACTIONS) {
    ...
    if (top.similarity >= 0.55 && nameOverlap >= 0.5) {
      results.push({
        ...
        stdlTreatmentItemId: top.szotarItem.id,
        stdlTreatmentItemName: top.szotarItem.name,
        conditions: {},
    ...
    if (!apiKey) {
      ...
      for (const p of batch) {
        results.push({
          ...
          stdlTreatmentItemId: p.topItem.szotarItem.id, stdlTreatmentItemName: p.topItem.szotarItem.name,
...
[{"action_index": 1, "pick": <sorszám vagy 0>, "confidence": <0.0-1.0>}, ...]`;
    ...
    for (let i = 0; i < batch.length; i++) {
      ...
      if (llmPick && llmPick.pick > 0 && llmPick.pick <= p.candidates.length) {
        ...
        results.push({
          atomicActionSlug: p.action.slug, atomicActionName: p.action.nameHu,
          stdlTreatmentItemId: chosen.id, stdlTreatmentItemName: chosen.name,
  ...
  for (const r of results) {
    const uses = szotarUsage.get(r.stdlTreatmentItemId) || [];
    uses.push(r.atomicActionSlug);
    szotarUsage.set(r.stdlTreatmentItemId, uses);
  }
  const collisions = [...szotarUsage.entries()]
    .filter(([, slugs]) => slugs.length > 1)
    .map(([id, slugs]) => ({
      szotarId: id,
      szotarName: results.find(r => r.stdlTreatmentItemId === id)?.stdlTreatmentItemName,
      actions: slugs,
    }));
  ...
  // 6. Save to Supabase
  // Clear old mappings
  await supabase.from('v2_clinic_mappings_stdl').delete().eq('telephely_id', telephelyId);

  // Insert new mappings (with top_candidates for review UI)
  const rows = results.map(r => ({
    telephely_id: telephelyId,
    stdl_treatment_item_id: r.stdlTreatmentItemId,
    stdl_treatment_item_name: r.stdlTreatmentItemName,
  ...
  if (rows.length > 0) {
    const { error: insertError } = await supabase.from('v2_clinic_mappings_stdl').insert(rows);
  ...
  for (const rule of VARIANT_RULES) {
    ...
    for (const variant of rule.variants) {
      ...
      if (bestItem && bestSim > 0.5) {
        ...
        await supabase.from('v2_clinic_mappings_stdl').insert({
          telephely_id: telephelyId,
          stdl_treatment_item_id: bestItem.id,
          stdl_treatment_item_name: bestItem.name,
  ...
  try {
    ...
    switch (operation) {
      ...
      case 'reset-telephely': {
        ...
        const { error: e1 } = await supabase.from('v2_clinic_mappings_stdl').delete().eq('telephely_id', telephelyId);
      ...
      case 'reset-profile': {
        ...
        const { error: e1 } = await supabase.from('clinic_treatment_items_stdl').delete().eq('telephely_id', telephelyId);
        if (e1) console.error('reset clinic_treatment_items_stdl:', e1);
