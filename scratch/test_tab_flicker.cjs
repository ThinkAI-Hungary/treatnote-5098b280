...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      const pepsziCard = cards.find(c => c.textContent.includes('Pepszi') && c.textContent.includes('Béla'));
      if (pepsziCard) {
        pepsziCard.click();
      } else {
        throw new Error("Could not find patient Card for Dr.Prof. Pepszi Béla");
      }
    });

    console.log("Waiting for patient profile page to load (URL change)...");
    await page.waitForFunction(() => {
      return window.location.pathname.includes('/patients/') && window.location.pathname.split('/').filter(Boolean).length >= 2;
    }, { timeout: 10000 });

    console.log("Patient profile loaded. URL:", page.url());

    // ─── STEP 1: Click Státuszkezelés ───
    console.log("\n--- Transition 1: Clicking Státuszkezelés ---");
    console.log("Waiting for Státuszkezelés link to appear...");
    await page.waitForFunction(() => {
      const links = Array.from(document.querySelectorAll('a'));
      return links.some(l => l.textContent.includes('Státuszkezelés'));
    }, { timeout: 10000 });
