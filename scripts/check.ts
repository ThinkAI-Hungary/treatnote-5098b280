...
const url = Deno.env.get("SUPABASE_URL") || "https://bpjzgapmoyhtgryglcke.supabase.co";
...
async function check() {
    ...
    const { data: comps, error: e3 } = await supabase.from('companies').select('id, name, seats, current_period_end').eq('id', 'b8e5c832-fa81-43fd-ae37-a14aeb1f6448').limit(5);
