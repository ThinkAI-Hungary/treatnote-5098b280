...
def kivesz_adatokat_vizitenkent(payload: Any) -> List[Dict[str, Any]]:
    ...
    if isinstance(payload.get("vizitek"), list):
        ...
        for it in items:
            ...
            if not fog_key.isdigit() and fog_key != "teljesszajureg":
                continue
    ...
    for vk in vizit_kulcsok:
        ...
        for tooth_key, tooth_obj in vt.items():
            if isinstance(tooth_key, str) and (tooth_key.isdigit() or tooth_key == "teljesszajureg") and isinstance(tooth_obj, dict):
        ...
        if a_schema:
            for tooth_key, tooth_obj in vt.items():
                if not (isinstance(tooth_key, str) and (tooth_key.isdigit() or tooth_key == "teljesszajureg") and isinstance(tooth_obj, dict)):
                    continue
            ...
            for cat, groups in vt.items():
                ...
                for g in groups:
                    ...
                    for f in fogak:
                        f_str = str(f).strip()
                        if not f_str.isdigit() and f_str != "teljesszajureg":
                            continue
...
"-"  # MUNKACSOPORT
"-"
async def kezeli_tutorial_modal_ha_van(page) -> None:
    """Kezeli az árajánlat oldalon felugró oktató (tutorial) ablakot a pontos ID-kkel."""
    try:
        modal = page.locator("#offerTutorialModal")
        
        try:
            await modal.wait_for(state="visible", timeout=1500)
        except Exception:
            return  # Nincs felugró ablak, azonnal továbblépünk
        
        if await modal.is_visible():
            naploz("TUTORIAL_MODAL detected")
            kiir("INFO", "Oktató ablak (Tutorial Modal) észlelve, megpróbálom bezárni...")
            
            # 1. Kipipáljuk a megadott checkboxot
            checkbox = page.locator("#offerTutorialModalDontShowAgainCheckbox")
            if await checkbox.count() > 0:
                await lep("TUTORIAL_MODAL check", lambda: checkbox.check(force=True))
            
            await page.wait_for_timeout(300)
                
            # 2. Rákattintunk a megadott Bezárás gombra
            finish_btn = page.locator("#offerTutorialModal_FinishButton")
            if await finish_btn.count() > 0:
                await lep("TUTORIAL_MODAL close btn click", lambda: finish_btn.click(timeout=2000, force=True))
            else:
                await page.keyboard.press("Escape")
                
            # 3. Megvárjuk, amíg az ablak ténylegesen eltűnik
            await modal.wait_for(state="hidden", timeout=4000)
            naploz("TUTORIAL_MODAL closed")
            kiir("DEBUG", "Oktató ablak sikeresen bezárva.")
            
    except Exception as e:
        naploz(f"TUTORIAL_MODAL hiba vagy kihagyva: {type(e).__name__}: {e}")
...
async def megy_uj_arajanlat_urlre_es_letrehoz(page, flexi_domain: str, paciens_id_szam: str) -> None:
    ...
    await kezeli_tutorial_modal_ha_van(page)
...
async def kivalaszt_fog(page, fog: str) -> None:
    if fog == "teljesszajureg":
        naploz("TELJES_SZAJUREG_SELECT")
        btn = page.locator('button.quadrant-selector-button[data-target="full-dentil"]:visible').first
...
async def megsem_fog(page, fog: str) -> None:
    if fog == "teljesszajureg":
        naploz("TELJES_SZAJUREG_DESELECT")
        btn = page.locator('span.dentil-nav-quadrant-delete-button-icon:visible').first
        await btn.scroll_into_view_if_needed(timeout=3000)
        await btn.click(timeout=3000)
        await page.wait_for_timeout(100)
    else:
...
async def vegrehajt_vizit(page, fog_kezelesek: Dict[str, List[str]], vizit_index: int) -> None:
    fogak = sorted(fog_kezelesek.keys(), key=lambda x: 0 if x == "teljesszajureg" else int(x))
    ...
    for fog_index, fog in enumerate(fogak):
        ...
        for kez_i, kez in enumerate(kez_lista):
            ...
            if fog == "teljesszajureg":
                await megsem_fog(page, fog)

            van_kov_fog = (fog_index + 1) < len(fogak)
            if van_kov_fog:
                kov_fog = fogak[fog_index + 1]
                await kivalaszt_fog(page, kov_fog)
                kovetkezo_fog_mar_kattintva = kov_fog

        if fog == "teljesszajureg" and kez_db == 0:
            await megsem_fog(page, fog)

        if fog != "teljesszajureg" and kovetkezo_fog_mar_kattintva is None:
            await megsem_fog(page, fog)
